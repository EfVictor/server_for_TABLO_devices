...
These devices do not have a static IP binding, so the server writes the MAC - IP binding to the info_tablo.txt file.
The server can also write data for the binding of cabinets and TABLO devices to the info_tablo_all_kab.json file and send data for display to a specific device or to all at once

For ease of configuration, the user has a web page tablo_seatings
--------
### Project structure:

1. node - server for storing images in ZIP or DICOMDIR format:
